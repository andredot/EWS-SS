...
### Key analysis points from EDA

The modeling approach should be bifurcated to account for the fundamentally different epidemiological behaviors of various health conditions, primarily to prevent the model from overfitting to random noise.
High-volume, communicable syndromes (such as respiratory or flu-like illnesses) exhibit strong auto-correlation and highly predictable annual cycles. These are modeled with the "full," highly flexible Generalized Additive Model (GAM), which utilizes cyclic annual splines (week_of_year), high-knot long-term trends ($k=20$)
Conversely, syndromes driven by sporadic, non-communicable, or idiosyncratic events lack these distinct seasonal patterns. If the full GAM is applied to them, the model "chases its own tail"—mistaking a random, short-term cluster of cases for the start of a seasonal wave, which creates erratic, bouncing thresholds that generate false alarms. To ensure stable and reliable alerting, these "flat" syndromes—specifically altered states of consciousness (alt_cosc), convulsions (conv), unexpected deaths (dec_gravi), drug intoxications (int_farm), and cutaneous rashes (rash_cut)—are evaluated using a "stiffer" GAM. 
This rigid model intentionally drops the annual seasonality parameter, heavily restricts the flexibility of the long-term trend ($k=3$), providing a flat, robust statistical ceiling that only triggers an alert during a mathematically genuine anomaly.
...
fit_surveillance_model <- function(df_subset) {
  ...
  s_name <- as.character(df_subset$sindrome[1])
  m <- fit_syndrome_gam(train_df = df_subset, syndrome_name = s_name)
  theta <- m$family$getTheta(TRUE)
  preds <- predict(m, type = "link", se.fit = TRUE)
  
  results <- df_subset |>
    mutate(
      expected = as.vector(exp(preds$fit)),
      # Approach A: Negative Binomial Quantiles
      p_val = as.vector(pnbinom(casi - 1, mu = expected, size = theta, lower.tail = FALSE)),
      limit_95_A = as.vector(qnbinom(0.95, mu = expected, size = theta)),
      limit_99_A = as.vector(qnbinom(0.99, mu = expected, size = theta)),
      status_A = case_when(
        casi > limit_99_A ~ "RED",
      ...
      std_residual = as.vector((casi - expected) / sqrt(expected + (expected^2 / theta))),
      status_B = case_when(
...
# The mapping remains exactly the same!
...
### Key analytical take-aways

While the current seasonal GAM effectively captures broad historical patterns—such as the expected winter rise in respiratory visits—it rigidly assumes that future epidemics will perfectly adhere to past calendar dates and volumes. In reality, a seasonal wave might arrive weeks early or establish a higher overall "new normal" for that specific year. Without an adaptive mechanism, a severe but slowly building flu season will continuously breach the static historical threshold, generating red alerts every single day for weeks. This creates severe "alert fatigue" for public health officials, rendering the system useless for detecting sudden, localized crises.

## 14-Day Adaptive Calibration
