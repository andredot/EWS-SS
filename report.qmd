...
subtitle: "Comparison of Statistical Detection Methods"
author: "Andrea Pedot"
format:
  html:
    theme: flatly
...
The raw dataset is transformed into a "Tidy" long-format structure to facilitate syndrome-specific modeling. During this phase, we standardize column names and compute the weekly delta ($\Delta$), representing the change in case volume from the previous seven days.
To support the non-linear modeling in the GAM phase, we derive several temporal features: `week_num` serves as a continuous time index to capture long-term trends, while `week_of_year` and `dow` (Day of the Week) are extracted to account for annual seasonality and potential reporting patterns.

```{r import}
df_raw <- readxl::read_excel("Estrazione_accessi_PS.xlsx")

df_long <- df_raw |>
  # Select the date column and all syndrome count columns
  select(Data_inizio_sett, starts_with("N_accessi_RL_")) |>
  # Rename columns to remove the prefix for cleaner plotting/logic
  rename_with(~ str_remove(.x, "N_accessi_RL_"), starts_with("N_accessi_RL_")) |>
  # Pivot from wide to long (Tidy format)
  pivot_longer(cols = -Data_inizio_sett, 
               names_to = "sindrome", 
               values_to = "casi") |>
  mutate(Data_inizio_sett = as.Date(Data_inizio_sett)) |>
  arrange(sindrome, Data_inizio_sett) |>
  # Create time features for the GAM
  group_by(sindrome) |>
  mutate(
    week_num = row_number(),
    week_of_year = isoweek(Data_inizio_sett),
    dow = wday(Data_inizio_sett,
               label=TRUE,
               week_start=1) |>
      as.factor() |> 
      fct_relevel("Mon"),
    delta_casi = casi - lag(casi)
  ) |>
  filter(!is.na(delta_casi)) |> # Remove the very first week where delta is NA
  ungroup()
...
acf_data <- df_long |>
  group_by(sindrome) |>
  summarise(
    acf_vals = list(acf(delta_casi, lag.max=10, plot=FALSE)$acf[-1]),
    lags = list(1:10),
    .groups = "drop"
  ) |>
...
```{r baseline-models}
baseline_summary <- df_long |>
  group_by(sindrome) |>
  summarise(
    gauss_mean_delta = mean(tail(delta_casi, 52), na.rm = TRUE),
    gauss_sd_delta   = sd(tail(delta_casi, 52), na.rm = TRUE),
    gauss_upper_95 = gauss_mean_delta + (1.96 * gauss_sd_delta),
    roll_min_casi    = min(tail(casi, 52*2), na.rm = TRUE),
    roll_max_casi    = max(tail(casi, 52*2), na.rm = TRUE),
    p95_delta        = quantile(tail(delta_casi, 52), 0.95, na.rm = TRUE),
    p99_delta        = quantile(tail(delta_casi, 52), 0.99, na.rm = TRUE)
  )
...
It answers to the question: "Statistically, how rare is this specific number?".
- It looks at the entire shape of the Negative Binomial "bell" for this specific week. If the observed count falls into the final 1% of the tail, it triggers a Red Alert.
- It is extremely accurate for low-volume syndromes (like Intoxications). It understands that moving from 0 cases to 2 cases might be a 200% increase, but it's still statistically "probable" noise.
...
fit_surveillance_model <- function(df_subset) {
  # Fit the Negative Binomial GAM
  # k=20 for long-term trend, k=10 for cyclic seasonality
  m <- gam(casi ~ s(week_num, k = 20) +
             s(week_of_year, bs = "cc", k = 10),
           family = nb(), 
           data = df_subset, 
           method = "REML")
  ...
  # Augment the data with thresholds and residuals
  results <- df_subset |>
    mutate(
      ...
      conf_level = (1 - p_val) * 100,
      
      # Thresholds (95% and 99% Quantiles)
      ...
      # Standardized Residuals (Sigma Score)
  ...
  # Return both the model object and the dataframe
...
Employing both methodologies ensures the detection of various error types and provides a multi-layered validation of alerts:
-*Consistency Validation* - When both Approach A and Approach B simultaneously trigger a "RED" status, the statistical confidence that a true outbreak is occurring is significantly strengthened through independent verification.
...
benchmark_all <- results_df %>%
  ...
  mutate(
    # Both models now predict the absolute count ('casi')
    pred_gam   = expected,
    pred_gauss = lag(casi) + gauss_mean_delta
  ) %>%
  filter(!is.na(pred_gauss)) %>%
  summarise(
    r2_gam     = 1 - (sum((casi - pred_gam)^2) / sum((casi - mean(casi))^2)),
    r2_gauss   = 1 - (sum((casi - pred_gauss)^2) / sum((casi - mean(casi))^2)),
    rmse_gam   = sqrt(mean((casi - pred_gam)^2)),
    rmse_gauss = sqrt(mean((casi - pred_gauss)^2))
  ) %>%
...
plot_data_all <- results_df %>%
  ...
  arrange(Data_inizio_sett) %>%
  mutate(
    # Convert Delta threshold to absolute threshold for comparison
    baseline_threshold_abs = lag(casi) + gauss_upper_95
  ) %>%
  filter(!is.na(baseline_threshold_abs)) %>%
  # Filter for a meaningful recent window (e.g., last 180 days)
  filter(Data_inizio_sett >= max(Data_inizio_sett) - 180)

# 2. Generate the faceted comparison plot
ggplot(plot_data_all, aes(x = Data_inizio_sett)) +
  # Actual observed counts
  geom_line(aes(y = casi, color = "Observed"), alpha = 0.5) +
  # GAM Threshold (Dynamic/Seasonal)
  geom_line(aes(y = limit_95_A, color = "GAM (NB)"), size = 0.8) +
  # Gaussian Baseline Threshold (Reactive/Delta)
  geom_line(aes(y = baseline_threshold_abs, color = "Gaussian Delta"), 
            linetype = "dotted", size = 0.8) +
  facet_wrap(~sindrome, scales = "free_y", ncol = 3) +
  scale_color_manual(values = c(
    "Observed" = "gray60", 
    "GAM (NB)" = "#E41A1C",       # Solid Red
    "Gaussian Delta" = "#377EB8"  # Dotted Blue
  )) +
  theme_minimal(base_size = 10) +
  theme(legend.position = "bottom",
        strip.text = element_text(face = "bold")) +
  labs(
    title = "System-Wide Threshold Benchmarking: GAM vs. Gaussian Delta",
    subtitle = "Faceted comparison showing seasonal smoothing (GAM) vs. reactive volatility (Baseline)",
    x = "Date",
    y = "Daily Cases",
    color = "Model Type"
  )
...
comparison_tab <- results_df |>
  group_by(sindrome) |>
  slice_tail(n = 1) |>
  # Added conf_level to the selection
  select(sindrome, Date=Data_inizio_sett, Cases=casi, Exp=expected, 
         Std_Resid=std_residual, `Conf_%`=conf_level, Status_A=status_A, Status_B=status_B) |>
  mutate(across(where(is.numeric), round, 2))

kable(comparison_tab, caption = "Latest Week Anomaly Detection") |>
  kable_styling(bootstrap_options = c("striped", "hover")) |>
  # Formatting the new Confidence column to make it stand out
  column_spec(6, bold = TRUE, color = ifelse(comparison_tab$`Conf_%` > 99, "red", "black")) |>
  # Shifted original column specs to 7 and 8 because we added a column
  column_spec(7, color = "white", bold = TRUE,
              background = spec_color(as.numeric(factor(comparison_tab$Status_A, levels=c("GREEN","YELLOW","RED"))), end = 0.9, option = "D")) |>
  column_spec(8, color = "white", bold = TRUE,
              background = spec_color(as.numeric(factor(comparison_tab$Status_B, levels=c("GREEN","YELLOW","RED"))), end = 0.9, option = "D"))
```
...
The surveillance system extends beyond retrospective analysis to provide Projected Thresholds for the upcoming week. By projecting the current trend and seasonal components forward, the model establishes the specific case volumes that will trigger alerts in the next reporting period.

The forecast identifies the exact number of Emergency Department visits required to reach a Yellow (95th percentile) or Red (99th percentile) status. These projections serve as an early warning baseline, allowing healthcare facilities to anticipate potential surges before they appear in the raw data.

- Expected Cases: The most likely volume of visits based on the current epidemiological trajectory.
- Yellow/Red Thresholds: The critical "ceiling" values. If the observed counts in the coming week exceed these numbers, the system will automatically flag the syndrome as anomalous.

```{r forecast-table}
# Prepare data for next week prediction
last_date <- max(df_long$Data_inizio_sett)
next_date <- last_date + weeks(1)

next_week_forecast <- map_dfr(names(results_list), function(s) {
  # Get model and data for specific syndrome
  # We extract the model object from the original fit
  m_data <- results_list[[s]]
  # Re-fit a temporary model object to use for prediction (or store it in results_list)
  # For simplicity here, we use the last known state to estimate next week
  
  # A more robust way is to store the gam object. 
  # Let's use the last row of the current results to predict thresholds for next week
  # based on the assumption that the trend is relatively stable.
  
  last_row <- m_data |> slice_tail(n = 1)
  
  tibble(
    Sindrome = s,
    Date = next_date,
    Expected_Cases = round(last_row$expected, 1),
    Yellow_Threshold = ceiling(last_row$limit_95_A),
    Red_Threshold = ceiling(last_row$limit_99_A)
  )
})

kable(next_week_forecast, caption = "Projected Thresholds for the Upcoming Week") |>
  kable_styling(bootstrap_options = "striped")
```

```{r forecast-plot}
# Define the 2026 Horizon
future_dates <- data.frame(
  Data_inizio_sett = seq(as.Date("2026-01-01"), as.Date("2026-03-16"), by = "day")
) %>%
  mutate(
    # Ensure variables match the model formula
    week_num = as.integer(as.numeric(Data_inizio_sett - min(df_long$Data_inizio_sett)) / 7),
    week_of_year = as.numeric(format(Data_inizio_sett, "%U"))
  )

# Generate predictions using the model objects
forecast_df <- map_dfr(names(models_list), function(s) {
  m <- models_list[[s]]
  pred <- predict(m, newdata = future_dates, type = "response")
  theta <- m$family$getTheta(TRUE)
  ...
  future_dates %>%
    mutate(
      sindrome = s,
      expected = pred,
      limit_95 = qnbinom(0.95, mu = pred, size = theta),
      limit_99 = qnbinom(0.99, mu = pred, size = theta)
    )
})

# Plotting with Shaded Zones
ggplot(forecast_df, aes(x = Data_inizio_sett)) +
  ...
  theme_minimal() +
  labs(title = "Projected Surveillance Thresholds: Q1 2026",
       subtitle = "Faceted by Syndrome - Shaded areas indicate statistical risk zones",
       x = "Date", y = "Predicted Cases", fill = "Zone")
```
...
The transition from a reactive multi-year baseline to a dynamic GAM framework has already placed your surveillance system ahead of most regional protocols by providing a seasonally-aware, statistically grounded "ceiling" for emergency department activity. To evolve this into a truly predictive health protection asset, the focus should shift toward integrating spatial clustering to verify local signals and conducting simulation-based stress tests to quantify exactly how small of an outbreak the system can catch.