...
```{r import}
# Map the parameter to the Excel column prefix
prefix_map <- c(
...
# Get the prefix based on the parameter chosen
selected_prefix <- prefix_map[params$area]

df_raw <- readxl::read_excel("Estrazione_accessi_PS.xlsx")

df_long <- df_raw |>
  # Select the date column and columns starting with our dynamic prefix
  select(Data_inizio_sett, starts_with(selected_prefix)) |>
  # Rename columns to remove that specific prefix
  ...
  filter(!is.na(delta_casi)) |> 
  ungroup()

# Update the Introduction title dynamically
