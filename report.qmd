...
baseline_summary <- df_long |>
  group_by(sindrome) |>
  summarise(
    # 1. Historical Range (Rolling 2-year Max)
    # Serves as a 'Safety Ceiling' based on past extremes
    roll_max_casi    = max(tail(casi, 52*2), na.rm = TRUE),
    roll_min_casi    = min(tail(casi, 52*2), na.rm = TRUE),
    
    # 2. Naive Poisson Expectation
    # The simplest count baseline: the long-term average
    poisson_mean     = mean(casi, na.rm = TRUE),
    
    # 3. Percentile Benchmarks for Deltas
